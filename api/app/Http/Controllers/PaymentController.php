...
class PaymentController extends Controller
{
    ...
    public function process(Request $request)
    {
        ...
        try {
            ...
            // Card gateways (Stripe & Cardlink) carry a 30% commission fee.
            // All other gateways (e.g. crypto) revert to the base amount.
            $cardGateways = ['stripe', 'cardlink'];

            if (in_array($validated['gateway'], $cardGateways)) {
                $invoice->amount = $baseAmount * 1.30;
                
                // Stripe minimum check (0.50 units e.g. 50 cents)
                if ($validated['gateway'] === 'stripe' && $invoice->amount < 0.50) {
